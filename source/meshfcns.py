...
from params import tol,Lngth,Hght,realtime_plot,X_fine,Nx
from dolfin import *
...
def mesh_routine(w,mesh,dt):
    ...
    # finally, compute the minimum and maximum grounding line positions
...
def get_glines(F_s):
    ...
    if np.size(glines)>0:
        ...
        XL = 0              # Minimum grounding line
        XR = Lngth          # Maximum grounding line
...
def plot_surfaces(F_h,F_s,XL,XR):
    ...
    if realtime_plot == 'on':
        ...
        p1 = plt.fill_between(X/1000-0.5*Lngth/1000,y1=Gamma_s[:], y2=Gamma_h[:]-0.99*Hght,facecolor='aliceblue',alpha=1.0)
        ...
        plt.plot(np.array([XL/1000-0.5*Lngth/1000]),np.array([np.min(bed(X))-1.0]),marker=r'^',color='crimson',linestyle='None',markersize=10,label=r'$x_\pm$')
        plt.plot(np.array([XR/1000-0.5*Lngth/1000]),np.array([np.min(bed(X))-1.0]),marker=r'^',color='crimson',markersize=10)
